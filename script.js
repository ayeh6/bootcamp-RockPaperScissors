...
function rockPaperScissors() {
    let playerChoice = prompt("Pick Rock, Paper or Scissors using R, P or S");
    //check valid input
    ...
    let compChoice = choices[Math.floor(Math.random()*3)];      // Math.floor(Math.random()*3) returns a random integer from 0-2
    alert(`Computer chose ${compChoice}`);
    let resultCheck = checkWinner(playerChoice, compChoice);
